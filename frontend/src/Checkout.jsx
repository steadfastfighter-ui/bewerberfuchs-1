...
export default function Checkout({ goDashboard, selectedProduct }) {
  ...
  async function unlockAdmin() {
    try {
      ...
      if (data.success) {
        window.location.href = `/?success=true&product=${selectedProduct || "bundle"}`;
        return;
      }

      alert("Nicht autorisiert.");
    } catch {
      alert("Admin Prüfung fehlgeschlagen.");
    }
  }
              ...
              <div className="rounded-3xl sm:rounded-[32px] bg-[#111827] p-6 sm:p-8 h-full">
                ...
                <div className="relative z-10">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="w-full py-5 rounded-2xl bg-orange-500 hover:bg-orange-400 hover:scale-[1.01] active:scale-[0.99] transition-all duration-300 text-white text-xl sm:text-2xl font-extrabold shadow-[0_0_40px_rgba(255,120,0,0.35)] disabled:opacity-60"
                  >
                    {loading ? "Stripe wird vorbereitet..." : current.button}
                  </button>
